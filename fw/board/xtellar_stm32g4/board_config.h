...
{
  // Preserve the explicit GPIO clock setup required by PF1 on this board.
  __HAL_RCC_GPIOA_CLK_ENABLE();
  __HAL_RCC_GPIOB_CLK_ENABLE();
  __HAL_RCC_GPIOF_CLK_ENABLE();

  hal::Stm32Spi::Options options;
  options.mosi = PA_11;
  options.miso = PA_10;
  options.sck = PF_1;
  options.cs = PB_7;
  options.frequency = 6000000;
  options.width = 16;
  options.mode = 0;
  return options;
}

inline device::Ma600::Config Ma600Config()
{
  device::Ma600::Config config;
  // 14 pole-pairs @ 200 rad/s ⇒ ~445 Hz electrical. 160 us keeps noise down
  // without consuming the high-speed torque budget.
  config.filter_us = 160;
  return config;
}
...
inline math::foc::FocController::Options FocControllerOptions()
{
  ...
  // delay. Keep this tied to Ma600Config().filter_us so the two cannot drift
  ...
  options.phase_lead_s =
      ...
      static_cast<float>(Ma600Config().filter_us) * 1.0e-6f;
  return options;
}
