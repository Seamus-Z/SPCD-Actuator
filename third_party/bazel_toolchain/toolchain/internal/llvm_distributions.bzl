...
_llvm_distributions = {
    ...
    # 20.1.8 - new naming/packaging scheme: distro-agnostic glibc Linux
    # builds, no libtinfo dependency. macOS-X64 (Intel) is no longer shipped
    # in 20.x; only Apple Silicon. No Windows tarball entry here because
    # upstream only ships an NSIS installer (.exe) for Windows under this
    # naming, which the rest of the toolchain cannot unpack.
...
_llvm_distributions_base_url = {
    ...
    "20.1.8": "https://github.com/llvm/llvm-project/releases/download/llvmorg-",
}
