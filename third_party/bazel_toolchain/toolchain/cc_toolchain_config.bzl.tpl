...
def _impl(ctx):
    ...
    cxx_builtin_include_directories = [
        ...
        # New-scheme tarballs (LLVM 19+) place per-target libc++ headers
        # (notably __config_site) under include/<triple>/c++/v1.
        "%{toolchain_path_prefix}include/" + target_system_name + "/c++/v1",
        # LLVM 16+ uses lib/clang/<major>; older versions use lib/clang/<full>.
        # Both are declared so this template works across versions.
        "%{toolchain_path_prefix}lib/clang/%{llvm_version}/include",
        "%{toolchain_path_prefix}lib/clang/%{llvm_major_version}/include",
        "%{toolchain_path_prefix}lib64/clang/%{llvm_version}/include",
        "%{toolchain_path_prefix}lib64/clang/%{llvm_major_version}/include",
    ]
    if (ctx.attr.cpu == "k8"):
        cxx_builtin_include_directories += [
