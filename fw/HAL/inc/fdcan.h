  ...
  enum class FilterAction
{
  ...
  struct Filter
  {
    uint32_t id1 = 0;
    uint32_t id2 = 0;
    FilterMode mode = FilterMode::RANGE;
    FilterAction action = FilterAction::DISABLE;
    FilterType type = FilterType::STANDARD;
  };
  ...
  struct FilterConfig
  {
    FilterAction global_std_action = FilterAction::ACCEPT;
    FilterAction global_ext_action = FilterAction::ACCEPT;
  ...
  struct SendOptions
  {
    Override bitrate_switch = Override::DEFAULT;
    Override fdcan_frame = Override::DEFAULT;
    Override remote_frame = Override::DEFAULT;
    Override extended_id = Override::DEFAULT;
  ...
  bool Send(uint32_t dest_id, std::string_view data, const SendOptions& send_options);

  // Returns true when a frame was copied into |data|.
  // |out_len| receives the decoded payload length (0..64).
  bool Poll(FDCAN_RxHeaderTypeDef* header, uint8_t* data, size_t max_len, size_t* out_len);
