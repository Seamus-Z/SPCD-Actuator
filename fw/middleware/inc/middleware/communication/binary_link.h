...
{
 public:
  // Return status byte for ACK (protocol::xt_can::kStatus*).
  ...
  void SendTelemetry(const protocol::xt_can::Telemetry& telem);
  void SendEncTelem(const protocol::xt_can::EncTelem& enc);
  void SendCalTelem(const protocol::xt_can::CalTelem& cal);
  void SendCtrlReply(const protocol::xt_can::CtrlReply& reply);
  void SendAck(uint8_t cmd, uint8_t seq, uint8_t status);
  void SendInfo(const protocol::xt_can::Info& info);
  bool SendConf(const void* data, size_t len);
  bool SendSnapMeta(const protocol::xt_can::SnapMeta& meta);
  bool SendSnapData(const protocol::xt_can::SnapData& data);

  uint8_t node_id() const { return node_id_; }
  uint16_t cmd_id() const { return protocol::xt_can::CmdId(node_id_); }
  uint16_t tel_id() const { return protocol::xt_can::TelId(node_id_); }
